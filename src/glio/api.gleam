...
import gleam/uri
import glio.{
  type ApiResponse, type ClioToken, type ClioYielder, type MyApp, ClioYielder,
  MyApp, TokenNotRenewed, TokenRenewed,
}
...
pub fn build_my_app(
  ...
  Ok(MyApp(my_apps_clio_id, my_apps_clio_secret, authorization_redirect_uri))
}
  ...
  case api_pure.url_to_request(clio_api_url) {
    Ok(base_api_request) -> {
      ...
      let api_response_result =
        api_impure.make_api_request(my_app, token, api_request_with_query)
...
pub fn fetch_one_page(
  ...
  let this_page_return = case api_response {
    TokenNotRenewed(_) -> TokenNotRenewed(this_page_of_data)
    TokenRenewed(_, new_token) -> TokenRenewed(this_page_of_data, new_token)
  }
  Ok(ClioYielder(this_page_return, prev_func, next_func))
}
